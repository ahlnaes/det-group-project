...
> [!What is FFT?]
> A fast Fourier transform (FFT) is an [[Algorithms|algorithm]] that computes the discrete Fourier transform ([[Discrete Fourier Transform|DFT]]) of a sequence, or its inverse (IDFT). A Fourier transform converts a signal from its original domain (often time or space) to a representation in the frequency domain and vice versa. 
...
This reduces complexity from $O(N²)$[^1] to $O(N \log_2 N)$[^2].
...
[^1]: [[O(N exp(2)) algoritmer]]
[^2]: [[O(N log N) algoritmer]]
